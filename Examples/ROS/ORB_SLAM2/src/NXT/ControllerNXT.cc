...
// Socket headers used for bluetooth connection
...
ControllerNXT::ControllerNXT():mbFinishRequested(false){}
...
{   
    ...
    if ( (write(nxtSocket, cmdLM, 15) < 0 ) || (write(nxtSocket, cmdRM, 15) < 0) ) {
        perror("error sending setoutputstate command ");
        return -1;
    }
    return 0;
}

// TODO: get distance from SLAM map or from boundary
int ControllerNXT::GetDistance()
{
    int distance = 0;
    cout << "Distance input: ";
    cin >> distance;
...
// TODO: get angle from SLAM map or from boundary (-180:180)
int ControllerNXT::GetAngle()
{
    int angle = 0;
    cout << "Angle input: ";
    cin >> angle;
...
{
    int result;
    const int refdist = 20;  // reference distance = 20cm
    int msrdist;             // measured distance
    const float gain = -5.0; // proportional gain (TODO:adjust later)
    int error;
    float power;
...
int ControllerNXT::TurnController()
{
    int result;
    const int refang = 0;    // reference angle = 0 deg
    int msrang;              // measured angle
    const float gain = -2.0; // proportional gain (TODO:adjust later)
    int error;
    float power;
...
{
    cout << "Finish of controllerNXT requested" << endl;
    unique_lock<mutex> lock(mMutexFinish);
    mbFinishRequested = true;
}

bool ControllerNXT::CheckFinish()
{
    unique_lock<mutex> lock(mMutexFinish);
...
void ControllerNXT::Run()
{
    ...
    while(1)
    {
        ...
        // if ( (result = SetOutput(powerLM, powerRM)) < 0 ) {
        //     break;
        // }
...
void ControllerNXT::OldRun()
{
    ...
    pangolin::CreateWindowAndBind("MyViewer",w+wui,h);
    ...
    // TEST: new window (DONT use cv window with Viewer:bUseViewer)
    cv::namedWindow("ORB-SLAM2: My Test Frame");
...
void ControllerNXT::LoadCameraPose(const cv::Mat &Tcw)
{
    if(!Tcw.empty())
    {
...
void ControllerNXT::PrintStatus(const int &status, const bool &bLocMode, cv::Mat &im)
{
    if(!bLocMode)
    {
        switch(status)
...
void ControllerNXT::AddTextToImage(const string &s, cv::Mat &im, const int r, const int g, const int b)
{
    int l = 10;
...
void ControllerNXT::DrawImageTexture(pangolin::GlTexture &imageTexture, cv::Mat &im)
{
    if(!im.empty())
    {
...
void ControllerNXT::DrawCube(const float &size,const float x, const float y, const float z)
{
...
void ControllerNXT::DrawPlane(Plane *pPlane, int ndivs, float ndivsize)
{
    glPushMatrix();
    pPlane->glTpw.Multiply();
...
void ControllerNXT::DrawPlane(int ndivs, float ndivsize)
{
...
void ControllerNXT::DrawTrackedPoints(const std::vector<cv::KeyPoint> &vKeys, const std::vector<MapPoint *> &vMPs, cv::Mat &im)
{
...
Plane* ControllerNXT::DetectPlane(const cv::Mat Tcw, const std::vector<MapPoint*> &vMPs, const int iterations)
{
    // Retrieve 3D points
